...
class WsHub {
  constructor(server) {
    ...
    // Slower floor for auth failures: no hammering a daemon that just
    // rejected the credentials, but recovery once they are fixed.
    this.authRetryDelay = 60000;
    this.pingTimer = null;
  ...
  async connect() {
    ...
    try {
      ...
      if (status === 401 || status === 403) {
        this.reconnectDelay = Math.max(this.reconnectDelay, this.authRetryDelay);
      }
    ...
    ws.on("unexpected-response", (_req, res) => {
      // With this listener attached, ws does NOT abort the handshake or
      // emit 'close' — every cleanup + reconnect step happens here, for
      // every status, or the hub wedges until redeploy.
      const status = res.statusCode;
      this.connecting = false;
      if (status === 401 || status === 403) {
        // A rotated token or expired session must not brick the hub:
        // keep retrying, but slowly, and clear a stale session so the
        // next attempt logs in again.
        if (this.sessionClient) this.sessionClient.cookies.clear();
        this.reconnectDelay = Math.max(this.reconnectDelay, this.authRetryDelay);
        this.notifyError(`ws authentication failed (HTTP ${status}); retrying in ${Math.round(this.reconnectDelay / 1000)}s`);
      } else {
        this.notifyError(`ws handshake rejected (HTTP ${status})`);
      }
      try {
        ws.terminate();
      } catch (_) {
    ...
    ws.on("close", (code) => {
      ...
      this.pingTimer = null;
      this.ws = null;
      // In-flight calls can never get their result frame now; fail
      // them immediately instead of letting each run into its timeout.
      for (const [, pending] of this.pendingCalls) {
        try {
  ...
  handleFrame(frame) {
    ...
    if (frame.op === "subscribed" || frame.op === "unsubscribed") {
      this.notifyControl({ op: frame.op, topics: frame.topics });
      return;
    }
