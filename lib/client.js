...
// parseApiVersion splits the daemon's semver `api_version` ("3.1.0")
...
class OpenccuLoomClient {
  ...
  buildURL(url, params) {
    ...
    if (params && typeof params === "object") {
      ...
      for (const [k, v] of Object.entries(params)) {
        if (v == null) continue;
        if (Array.isArray(v)) {
          for (const item of v) if (item != null) qs.append(k, String(item));
        } else {
  ...
  async rawRequest(cfg) {
    ...
    // fetch forbids GET/HEAD bodies; a payload on a generic GET (the
    // classic inject -> api-node wiring) must not abort the request.
    const hasBody = cfg.data !== undefined && method !== "GET" && method !== "HEAD";
...
function describeError(err) {
  ...
  if (cause) return `${err.message || String(err)}: ${cause}`;
  return err.message || String(err);
}

// describeCause unwraps undici's nested network errors ("fetch failed"
// wraps ECONNREFUSED / DNS / TLS causes, sometimes as AggregateError)
// so operators can tell a wrong port from a bad certificate.
function describeCause(cause) {
  if (!cause) return "";
  if (Array.isArray(cause.errors) && cause.errors.length > 0) {
    return cause.errors.map((e) => describeCause(e) || e.message).filter(Boolean).join("; ");
  }
  const inner = describeCause(cause.cause);
  const own = cause.code || cause.message || "";
  if (inner && own && !inner.includes(own)) return `${own} (${inner})`;
  return own || inner;
}
